...
/**
 * Works out how many decimal places of a value hold up against digits of Pi
 * derived by a route with nothing in common with whatever produced it
 * @param value The value being checked
 * @param precision How many decimal places were asked for
 * @return How many of those places are confirmed, or -1 when the value is not
 *         Pi at all rather than merely short of places
 */
static int confirmed_decimal_places(const mpf_class &value, int precision)
{
  ...
  mp_exp_t hex_exp;
  string hex_all = value.get_str(hex_exp, 16, 0);
  ...
  return confirmed_decimals;
}

/**
 * Adds the value to a report, cut down to the room available, with a caret on
 * the line beneath pointing at the first digit that did not hold up
 * @param report Gains one line, or two when there is a digit to point at
 * @param label What goes in front of the value on its line
 * @param value_str The value written out in full
 * @param available_width How much room is left for the value beside the label
 * @param mismatch_index Where the first wrong digit sits in value_str, or -1
 *        when every digit held up
 */
static void add_value_lines(AccuracyReport &report, const string &label,
                            const string &value_str, size_t available_width,
                            int mismatch_index)
{
  const string red = "\x1b[31m";
  const string reset_color = "\x1b[37m";

  // Nothing to point at, so the value has only to be got down to the width
  if (mismatch_index < 0)
  {
    string shown = value_str;
    if (value_str.length() > available_width)
    {
      shown = value_str.substr(0, available_width - 3) + "...";
    }

    report.add_line(label + shown);
    return;
  }
  ...
  // The whole value fits, so the caret sits under the digit where it already is
  if (value_str.length() <= available_width)
  {
    shown = value_str;
    caret_line = string(label.length() + mismatch_index, ' ') + red + "^" + reset_color;
  }
  // Too long to show whole, but the digit is near enough the front to survive
  // the trim, so the caret still lands where it started
  else if (mismatch_index < static_cast<int>(available_width - 4))
  {
    shown = value_str.substr(0, available_width - 3) + "...";
    caret_line = string(label.length() + mismatch_index, ' ') + red + "^" + reset_color;
  }
  // The digit is far enough along that trimming the tail would cut it off, so
  // keep a few from the front to place the value and then jump to where it is
  else
  {
    ...
    int context_start = mismatch_index - (context_len / 2);
    if (context_start + context_len >= static_cast<int>(value_str.length()))
    {
      context_start = value_str.length() - context_len;
    }

    shown = value_str.substr(0, prefix_len) + ellipsis
      + value_str.substr(context_start, context_len);

    int caret_pos = label.length() + prefix_len + ellipsis.length()
      + (mismatch_index - context_start);
    caret_line = string(caret_pos, ' ') + red + "^" + reset_color;
  }

  report.add_line(label + shown);
  report.add_line(caret_line);
}
                                   ...
                                   const string &method_name)
{
  ...
  const int confirmed_decimals = confirmed_decimal_places(calculated_pi, precision);

  // The wrong number altogether rather than the right one short of places, so
  // there is nothing to count and nowhere past the start worth pointing at
  if (confirmed_decimals < 0)
  {
  ...
  if (confirmed_decimals >= precision)
  {
    ...
    add_value_lines(result, calculated_pi_label, calculated_str, available_width, -1);
  ...
  add_value_lines(result, calculated_pi_label, calculated_str, available_width, mismatch_idx);
