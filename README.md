...
WPCP (Wii Pi Calculator Project) by MadCatMk2. It calculates the digits of pi
(π) on the Wii. It has since been rewritten in C++, modernized, and extended
...
You must have a functional devkitPro environment set up with `devkitPPC`,
`wii-dev`, and `libogc`. To set up the devkitPro devkitPPC PowerPC build
system, follow the instructions on the official devkitPro wiki:
...
`--recursive` flag is essential as it tells Git to download the required GMP
...
> **Note:** If you cloned without `--recursive`, go into the project folder and
> run `git submodule update --init --recursive` to fetch the GMP dependency.
...
1. Copy the `apps` folder from this repository to the root of your SD card.
   This will create a `/apps/WPCPP/` directory.
2. Copy the compiled `WPCPP.dol` file into the `SD:/apps/WPCPP/` folder and
   **rename it to `boot.dol`**.
3. Insert the SD card into your Wii and launch the program from the Homebrew
   Channel.
4. Follow the onscreen prompts.

<br>

![WPCPP 2.0 Screenshot](extras/wpcpp_screenshot.png)
...
If you have already cloned the project and want to pull the latest changes from
this repository, you must run two commands. The first pulls the main project
updates, and the second ensures the GMP submodule is updated to match what the
project specifies.
...
This project is an application that runs on the Wii via the Homebrew Channel.
It is not affiliated with, endorsed by, nor sponsored by the creators of the
Wii console nor the Homebrew Channel. All trademarks and copyrights are the
property of their respective owners.

This program is distributed under the terms of the GNU General Public License
version 3 (GPLv3). You can redistribute it and/or modify it under the terms of
